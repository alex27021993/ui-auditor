...
## `playbooks/` — починка, НЕ аудит

Папка `playbooks/` — отдельные инструкции по **ремонту** конкретных поломок (требуют
доступа к серверу/деплоя). Аудит-скилы read-only и их НЕ выполняют: их применяет
администратор сайта вручную. Сам аудит только диагностирует.
